...
def filter_authors_by_nationalities(curr_nationality) -> str:
    authors = Author.objects.filter(nationality=curr_nationality).order_by('first_name', 'last_name')
...
def filter_authors_by_birth_year(born_year_1: int, born_year_2: int) -> str:
    # Solution with __gte and __lte
    authors = (Author.objects.filter(birth_date__year__gte=born_year_1, birth_date__year__lte=born_year_2)
               .order_by('-birth_date'))

    # Solution with range()
    # authors = Author.objects.filter(birth_date__year__range=(born_year_1, born_year_2)).order_by('-birth_date')

    return '\n'.join(f"{a.birth_date}: {a.first_name} {a.last_name}" for a in authors)


def change_reviewer_name(curr_name: str, new_name: str) -> QuerySet:
